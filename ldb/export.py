...
class CSVSerializer(Serializer):
  ...
  def to_csv(self, data, options=None):
    ...
    try: 
      ...
      order = ['name','streetnumber','postcodecity','kixcode','street_name','house_number','address_2','address_3','postcode','city','country','email','phone_fixed','organization__name_prefix','organization__name','organization__name_short','organization__salutation','person__titles','person__initials','person__firstname','person__preposition','person__surname','person__postfix_titles','person__phone_mobile','person__gender','person__birthdate','person__ldap_username','person__netid','person__student__study','person__student__first_year','person__student__student_number','person__student__graduated','person__student__phone_parents','person__alumnus__study','person__alumnus__study_first_year','person__alumnus__study_last_year','person__alumnus__work_company']
...
class ExportResource(Resource):
  ...
  allowed_fields = {
    'entity': ['street_name', 'house_number', 'address_2', 'address_3', 'postcode', 'city', 'country', 'email', 'phone_fixed'],
    'organization': ['name_prefix', 'name', 'name_short', 'salutation'],
    'person': ['titles', 'initials', 'firstname', 'preposition', 'surname', 'postfix_titles', 'phone_mobile', 'gender', 'birthdate', 'ldap_username', 'netid'],
    'person__member': [],
  ...
  allowed_querysets = {
    ...
    'members':        Q(living_person, Q(person__member__date_from__isnull=False), Q(person__member__date_to__isnull=True)),
  ...
  def obj_get_list(self, bundle, **kwargs):
    ...
    if addresslist == 'off':
      ...
      objects = objects.filter(
        ...
        'organization__name_prefix', 'organization__name', 'organization__name_short', 'organization__salutation',
        'person__titles', 'person__initials', 'person__firstname', 'person__preposition', 'person__surname', 'person__postfix_titles',
      ...
      def getname(obj):
        if obj.get('organization__name'):
          ...
          name = "%s %s %s %s" % (firstname, obj.get('person__preposition', ''), obj.get('person__surname',''), obj.get('person__postfix_titles',''))
          name = re.sub("\s+", " ", name)
          return name.strip()
        else:
          return ""
